...
- **Player/Remote (MultiSync)** – one FPP **player** synchronises any number of FPP
  **remotes** with small sync packets, so each remote drives a nearby section of
  ...
  playlists and schedules. See [MultiSync](#multisync).
...
better solution (see [Backup, Restore and Proxies → Proxy Settings](#proxy-settings)).
...
Connect** to distribute them. See the [MultiSync](#multisync) chapter for the full page
reference.
